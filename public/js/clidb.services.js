	...
	function newDb(socket) {
		...
		service.api = {
			...
			get : function(classkey, itemkey, qid) {
				socket.emit('clidb.getitem', classkey, itemkey, qid);
			},

			getp : function(classkey, itemkey, path, qid) {
				socket.emit('clidb.getitemprop', classkey, itemkey, path, qid);
			},

			dlt : function(classkey, itemkey, qid) {
				//service.data[classkey][itemkey] = null; // <-- remove locally
				delete service.data[classkey][itemkey];
				socket.emit('clidb.deleteitem', classkey, itemkey, qid);
			},

			set : function(classkey, itemkey, value, qid) {
				socket.emit('clidb.setitem', classkey, itemkey, value, qid);
			...
			list : function(classkey, qid) {
				socket.emit('clidb.listclasses', classkey, qid);
			},

			getclass : function(classkey, qid) {
				socket.emit('clidb.getclass', classkey, qid);
			},

			schema : function(schemaName, qid) {
				var schema = tv4.getSchema(schemaName);
				if (!schema) socket.emit('clidb.getschema', schemaName, qid);
			...
			export : function(classkey, qid) {
				socket.emit('clidb.export', classkey, qid);
			}
		}
		...
		$rootScope.$on('socket.io.connected',function(){
			//socket.emit('clidb.getschema');
			socket.emit('clidb.getall');
		});


		socket.on('clidb.schema',function(err, id, schema, qid){
			if (err) {
		...
		socket.on('clidb.class',function(err, classkey, value, qid){
			$rootScope.$apply(function(){
		...
		socket.on('clidb.item',function(err, classkey, itemkey, value, qid){
			$rootScope.$apply(function() {
				if (!service.data[classkey]) service.data[classkey] = {};
				try {
		...
		socket.on('clidb.getitemprop', function(err, result, qid) {
			$rootScope.$apply(function() {
		...
		socket.on('clidb.setitem', function(err, classkey, itemkey, item, qid){
			$rootScope.$apply(function(){
		...
		socket.on('clidb.deleteitem', function(err, data, qid){
			$rootScope.$apply(function(){
		...
		socket.on('clidb.listclasses', function (err, result, qid) {
			$rootScope.$apply(function(){
				if (qid) linkCallback(err, result, qid);
			}, true);
		});

		socket.on('clidb.export', function(err, classkey, result, qid){
			$rootScope.$apply(function(){
		...
		/**
		 * export function - tell the server to write the specified file to disk
		 */
		service.export = function(classkey) {
			
			socket.emit('clidb.export', classkey);
		}

		socket.on('clidb.export', function() {
